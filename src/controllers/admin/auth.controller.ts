...
class AuthController{
    ...
    async createAdmin(req:ReqInterface,res:ResInterface)
    {
        try{
            const email = 'admin@mavarick.com';
            const password='Admin@123';
            const { error, message, data } = await AuthService.createAdmin({email,password});
            if (error) {
